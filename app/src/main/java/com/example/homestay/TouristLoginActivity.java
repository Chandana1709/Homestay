...
public class TouristLoginActivity extends AppCompatActivity {
                    ...
                    ParseUser.logInInBackground(emailEditText.getText().toString(), passwordEditText.getText().toString(), new LogInCallback() {
                        ...
                        public void done(ParseUser user, ParseException e) {
                            if (e ==null){
                                ...
                                if (user_owner == 1){ // check if the value is equal to 1 (or any other value that indicates verification)
                                    showAlert("Login Successful","Your email is verified successfully",false);
                                } else {
                                    showAlert("Login failed","Your not a tourist go to owner login",true);
                                }
                                //showAlert("Login Successful","Your email is verified successfully",false);
                            }else {
    ...
    private void startSignUpActivity(){
        Intent intent = new Intent(TouristLoginActivity.this, TouristRegActivity.class);
    ...
    private void ForgotPasswordActivity(){
        Intent intent = new Intent(TouristLoginActivity.this, TouristForgotPassword.class);
        startActivity(intent);
        finish();
    }
    private void startWelcomeActivity() {
        Intent intent = new Intent(TouristLoginActivity.this, TouristDashboardActivity.class);
        startActivity(intent);
        finish();
    }
